...
GameObject::Spaceship::Spaceship(Core::GameManagers& manager, const Utils::Vector2f& pos) : GameObject(manager), position(pos), collider(), physics(gManager.GetManager<Core::PhysicsManager>())
{
	...
	texture = resourceManager.GetOrLoad<Resources::Texture>(spaceshipTextureID, spaceshipTexturePath);
	shootingSound = resourceManager.GetOrLoad<Resources::Sound>(shootingSoundID, shootingSoundPath);
	bullets.reserve(150);
}
